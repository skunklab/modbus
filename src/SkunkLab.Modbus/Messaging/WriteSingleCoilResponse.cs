...
namespace SkunkLab.Modbus.Messaging
{
    [Serializable]
    [JsonObject]
    public class WriteSingleCoilResponse : ModbusMessage
    {
        private const MessageType type = MessageType.WriteSingleCoilResponse;
                

        public static WriteSingleCoilResponse Decode(byte[] message, ILogger logger = null)
        {
            if (message == null)
        ...
        public static WriteSingleCoilResponse Decode(string message)
        {
            return JsonConvert.DeserializeObject<WriteSingleCoilResponse>(message);
        }
