...
{
    public class ReadInputRegistersResponse : ModbusMessage
    {
        public ReadInputRegistersResponse()
        {
        }

        private const MessageType type = MessageType.ReadInputRegistersResponse;

        public static ReadInputRegistersResponse Decode(byte[] message, ILogger logger = null)
        {
            if (message == null)
            ...
            try
            {
                ...
                return new ReadInputRegistersResponse()
                {
                    ...
                    InputRegisters = GetValues(message, index, message[index - 1])
                };
            }
        ...
        public static ReadInputRegistersResponse Decode(string message)
        {
            ReadInputRegistersResponse response = JsonSerializer.Deserialize<ReadInputRegistersResponse>(message);
        ...
        [JsonPropertyName("inputRegisters")]
